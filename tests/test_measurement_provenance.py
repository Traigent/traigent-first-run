"""Require reproducible provenance for numbers reported as measurements.

The guard scans shipped first-run files, `GUIDE.md`, and `.github/`. It does
not try to prove that a reported result is correct; that would duplicate the
scorer. It instead requires a reader to be able to find the producer and
input that can reproduce the result.

A tracked path, real Python symbol, numeric/list fixture, or code fence counts
only when the note causally introduces it with wording such as `Source:`,
`produced by`, `through`, `from`, or `using`. Merely placing `GUIDE.md`,
`ROOT`, or `n: 1` beside an unrelated result does not satisfy the rule. Paths
and symbols are resolved against tracked files and parsed definitions, so
invented references fail too.

The claim grammar covers passive and active measurement language and numeric
results written either as prose or inline code. Numeric stage labels are
masked before claim extraction because `built in stage 3 and verified in
stage 5` describes workflow order, not measured values. Honest statements
about an instrument, an unmeasured state, or a displayed card shape remain
outside the rule.

Issue #181's mirror sweep is intentionally review evidence rather than a broad
prose ban: exact dataset thresholds formerly hedged as `about` or `roughly`
were made exact, while the small-sample intervals, runtime estimates, and
effects that can reverse remain hedged and are pinned below. Most uses of
`about` in this package are ordinary prepositions, so treating the word itself
as a violation would mostly reject honest prose.

`tests/` and `tools/` are deliberately outside the corpus: their numbers sit
beside the assertions that produce them, while guidance-budget ledger entries
already have a dedicated guard. Commit messages and pull-request bodies are
not present in a working tree and remain reviewer responsibilities.

`UNDERIVABLE` is the narrow escape hatch for a claim whose source cannot live
in this tree. Each entry quotes one exact shipped claim, must still exist, and
must still fail the normal scanner; stale or overbroad permissions therefore
fail the suite.

Run the guard directly with:

    python -c "import sys; sys.path.insert(0, 'tests'); import test_measurement_provenance as m; print(m.scan_tree())"
"""
...
_CAUSAL = re.compile(
    r"(?:source|producer)\s*:|\b(?:through|from|using|produced by|read with)\b",
    re.IGNORECASE,
)
_STAGE_LABEL = re.compile(r"\bstages?\s+\d+(?:\s*(?:,|and)\s*\d+)*", re.IGNORECASE)
_NUMERIC_CODE = re.compile(r"[-+]?\d[\d_]*(?:\.\d+)?(?:s|ms|m|h|k|M|B|kb|MB|GB|%)?")
...
def provenance(
    ...
    def causally_introduced(start: int) -> bool:
        return bool(_CAUSAL.search(unit[max(0, start - 160) : start]))

    for match in _FENCE.finditer(unit):
        region = match.group(0)
        if causally_introduced(match.start()):
            return "a causally introduced code block"
    for match in _CODE_SPAN.finditer(_FENCE.sub(" ", unit)):
        region = match.group(0)
        if _BINDING.search(region.strip("`")) and re.search(
            r"\b(?:on|over)(?:\s+the)?\s*$|\b(?:input|fixture)\s*(?::\s*)?$",
            unit[max(0, match.start() - 80) : match.start()],
            re.IGNORECASE,
        ):
            return f"the input fixture {region.strip()!r}"
    for literal in _BRACKETED.findall(unit):
        if _BINDING.search(literal):
            start = unit.find(literal)
            if re.search(
                r"\b(?:on|over|input|fixture)\b",
                unit[max(0, start - 80) : start],
                re.IGNORECASE,
            ):
                return f"the input fixture {literal.strip()!r}"
    for line in unit.splitlines():
        if line[:2].isspace() and line.strip() and _BINDING.search(line):
            start = unit.find(line)
            if causally_introduced(start):
                return f"the input fixture {line.strip()!r}"
    for match in _PATHLIKE.finditer(unit):
        if match.group(0) in paths and causally_introduced(match.start()):
            return f"the producer path {match.group(0)!r}"
    for region_match in _CODE_SPAN.finditer(_FENCE.sub(" ", unit)):
        region = region_match.group(0)
        if not causally_introduced(region_match.start()):
            continue
        for match in _IDENTIFIER.finditer(region.strip("`")):
            ...
            for candidate in (name, name.split(".")[0], name.rsplit(".", 1)[-1]):
                if candidate in symbols:
                    return f"the producer symbol {name!r}"
...
def _masked(unit: str) -> str:
    """Blank code identifiers while retaining numeric results written as code."""
    blanked = _FENCE.sub(lambda match: " " * len(match.group(0)), unit)
    blanked = _STAGE_LABEL.sub(lambda match: " " * len(match.group(0)), blanked)

    def mask_span(match: re.Match[str]) -> str:
        inner = match.group(0)[1:-1].strip()
        return (
            f" {inner} "
            if _NUMERIC_CODE.fullmatch(inner)
            else " " * len(match.group(0))
        )

    return _CODE_SPAN.sub(mask_span, blanked)
...
def _reports_a_measurement(sentence: str) -> bool:
    ...
    for index, raw in enumerate(tokens):
        ...
        if following and following[0] in SCOPE_PREPOSITIONS:
            return True
        previous = tokens[index - 1].strip(_PUNCTUATION).casefold() if index else ""
        later = [
            token.strip(_PUNCTUATION).casefold()
        ...
        if (
            "at" in later
            and previous not in {"a", "an", "the", "is", "was", "were", "be", "been"}
            and not prior.intersection({"no", "not", "never", "nothing"})
        ):
            return True
        if word in {"confirmed", "benchmarked"} and "at" in later:
            return True
    return False
...
class MeasuredNumbersAreReDerivableTests(unittest.TestCase):
    ...
    UNSOURCED_AND_FIXED = (
        ...
        (
            ...
            "The card line was swept exhaustively over 88 704 states through "
            "`score_evaluation`.",
        ),
        (
        ...
        (
            ...
            "Measured on one 10 000-configuration space using fixture "
    ...
    def test_a_failure_names_the_file_and_the_number(self) -> None:
        ...
        missing_path = "skills/traigent-first-run/references/" + "x.md"
        found = self._scan(planted, missing_path)
...
class OnlyAReportedMeasurementIsAskedTests(unittest.TestCase):
    ...
    def test_the_report_shapes_are_asked(self) -> None:
        for sentence in (
            ...
            "We measured the agent pillar at 77.",
            "The benchmark measured the agent pillar at 77.",
            "Our sweep measured the agent pillar at 77.",
            "This benchmark confirmed the agent pillar at 77.",
            "Measured on this scorer, the pillar reads `77`.",
            "The card line was swept exhaustively over 221 760 states.",
    ...
    def test_the_vague_claim_sweep_keeps_only_honest_hedges(self) -> None:
        """The non-numeric half of #181 was classified, not blanket-banned."""
        shipped = "\n".join(
            (ROOT / name).read_text(encoding="utf-8")
            for name in tracked_files(IN_SCOPE_PATHSPEC)
            if name.endswith(IN_SCOPE_SUFFIXES)
        )
        # These were exact >100 selection conditions disguised as estimates.
        # Assemble the old spellings so this test is not its own offender.
        for old in ("about " + "100 usable rows", "roughly " + "100 usable rows"):
        ...
        for honest in (
            ...
            with self.subTest(honest=honest):
                self.assertIn(honest, shipped)

    def test_a_numeric_result_shown_as_code_is_still_a_claim(self) -> None:
        self.assertNotEqual(
            self._scan("Measured on this scorer, the pillar reads `78`."), []
        )

    def test_stage_labels_are_not_measurement_results(self) -> None:
        self.assertEqual(
            self._scan(
                "The integration is built in stage 3 and verified against the SDK in stage 5."
            ),
            [],
        )
...
class WhatCountsAsProvenanceTests(unittest.TestCase):
    ...
    def test_a_tracked_path_is_provenance_and_an_invented_one_is_not(self) -> None:
        self.assertIsNotNone(
            self._provenance("Measured on 12 rows. Source: references/run-safety.md.")
        )
        missing_path = "references/" + "no-such-file.md"
        self.assertIsNone(self._provenance(f"Measured on 12 rows, see {missing_path}."))

    def test_a_real_symbol_is_provenance_and_a_plausible_name_is_not(self) -> None:
        self.assertIsNotNone(
            self._provenance("Measured through `EXCLUDED_KNOB_REASONS`: 77.")
        )
        self.assertIsNone(
    ...
    def test_a_bracketed_literal_and_an_indented_block_are_fixtures(self) -> None:
        ...
        self.assertIsNotNone(
            self._provenance("Measured using fixture:\n\n    max_trials: 12\n")
        )

    def test_a_code_fence_is_a_fixture(self) -> None:
        self.assertIsNotNone(
            self._provenance(
                'Measured using fixture:\n\n```json\n{"max_trials": 12}\n```\n'
            )
        )

    def test_irrelevant_decoration_is_not_provenance(self) -> None:
        for decoration in (
            ...
            with self.subTest(decoration=decoration):
                self.assertIsNone(self._provenance(decoration))
...
class TheNoteWindowDoesNotBorrowProvenanceTests(unittest.TestCase):
    """A neighbouring fixture for another claim cannot source this number."""
    ...
    def test_the_defect_as_it_shipped_is_caught(self) -> None:
    ...
    def test_the_neighbouring_fixture_sources_neither_claim_without_a_causal_link(
        self,
    ) -> None:
        first, second = self.UNREACHABLE_78_AS_IT_SHIPPED.split("#\n")
        self.assertIsNone(provenance(first, self.paths, self.symbols))
...
class EscapeHatchStaysHonestTests(unittest.TestCase):
    ...
    def test_an_entry_for_a_sentence_that_is_no_longer_refused_fails(self) -> None:
        ...
        """
        quote = (
            "Measured through `EXCLUDED_KNOB_REASONS`, the agent "
