...
the arrangement the run-log validator already uses two hundred lines below. That
is what holds the raise to six lines: what each exit means, the five properties
read from inside the boundary, and the four the check cannot reach - resource
limits, per-candidate disposability, descendant teardown, and any host path it
was never told about - all live where they cannot drift away from the code that
decides them.
...
The path is narrow and the cost is sized to it. A scorer that executes anything
is roughly one project shape in forty-six, so none of this enters the resident
documents and that figure is unmoved. A reader who never brings such an
evaluator pays six lines in a reference they already load, inside a section that
was two thousand bytes of contract nobody could act on; the reader who does
bring one is told, at the moment their own evaluator is parsed, which command
settles it.
