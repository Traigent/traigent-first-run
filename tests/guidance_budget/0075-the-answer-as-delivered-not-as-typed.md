# 0075 - the answer as delivered, not as typed
...
are typed by the assistant; the generated ones are built out of the expected
answer, which the assistant also typed. Between the model and the scorer sits a
step nobody typed a probe for - whatever the agent does to a reply before
handing it over - and neither set has ever crossed it. In one walkthrough a chat
model returned its query inside a code fence and twelve configurations scored
zero, which ranks nothing at all. In the other an extraction routine kept the
last statement keyword it could find, which is later in the string than the
outer one whenever a query holds a subquery, so a right answer arrived as an
unbalanced fragment reading a different table. One row in eighteen shifted every
configuration by roughly the width of the whole spread, so what came back was
not a low board but a wrong one.

The second is the instructive half, because the shape that broke it was already
sitting in the case matrix. A subquery had been written into the probes and was
