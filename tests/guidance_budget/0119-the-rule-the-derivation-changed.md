# 0119 - the rule the derivation changed
...
prescribing the shape the change was made to stop paying for. An agent following it would declare a
tool whose name sits in a table nothing reads, and the customer's pillar would fall for a reason the
guide had called fine.

Nothing failed over it, and nothing could have. The scripts carry no copy of these sentences, the
byte budget does not read English, and no contradiction registry knows this pair - the change was
invisible in its own diff because its diff touched neither document. That is the shape this
repository's whole-document rule exists for, and this entry is the third time in one chain that the
rule has earned its cost.

What the bytes buy in the reference is a rule an author can follow without guessing: the refusal is
unchanged and still fires on a name the file never mentions, presence alone survives that refusal
and earns nothing, and credit needs the callable to reach the name - directly, or through one hop to
a module-level assignment the callable itself names. Stated with its limit attached, because the
