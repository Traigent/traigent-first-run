...
# The disclosure rules are a tool, not a test fixture: the pre-publish scan needs
# them too, and a second copy of the digest list would itself be the leak.
...
class SkillPackageTests(unittest.TestCase):
    ...
    def test_readme_discloses_pinned_sdk_license_terms(self) -> None:
        ...
        # above, and the whole suite passes while README still names the terms
        # of a release nobody installs. Retiring the behaviour lock made that
        # sequence shorter, not safer - it used to end in a relock step, and
        # now not even that ritual stands between the bump and a green run.
        # The same class as that test's prior art, applied to the document
        # rather than the pin.
    ...
    # What the assistant is now mandated to bring to the opening gate: its own
    # read of the agent's source. Two settings with two values each, cited, is
    # the smallest document that establishes a space at all.
    OPENING_READ = {
        "knobs": {
            "model": {
    ...
    def _opening_card(
        self, evaluation: int, dataset: int, agent_facts: object = None
    ) -> tuple[int, str]:
        """Score one modelled opening card.
        ...
        built by `score_agent` rather than hand-written here. That is the point
        of building it this way: whatever that branch produces (a cap, a
        ceiling, a withheld check, nothing) is what the sweep sees, so a change
    ...
    def test_the_band_sentence_matches_what_calibration_can_actually_move(
        ...
        #201 does not remove that ceiling. It removes the reason the ceiling
        applied to everybody: the assistant now READS the agent at the opening
        gate, so the card that used to be defined by a missing document is
        defined by what the read found. Measured on the same 121 shapes with
        the same helper, now swept twice - once with the read the guide
        mandates and once with the state where nothing about the agent reached
        the score at all. The first tops out at 93 EXCELLENT with no cap. The
        second is held at 45, which is the same number a settings document
        declaring an empty space gets, because it is the same evidence about
        the same search space.

        An earlier revision of this branch deleted the second ceiling too, and
        the sweep is why that is not what shipped: it put the state where
        nothing was supplied at 75, thirty points above the state where a
        customer handed over a document saying the same thing.

        The property asserted is still derived from the sweep rather than
        restated from the paragraph, so a ceiling landing on the conformant
        opening card fails here instead of quietly outdating the README.
        """
    ...
    def test_no_internal_tooling_is_named_in_this_public_package(self) -> None:
        ...
        """
        # Every rule now lives in tools/disclosure_scan.py, imported rather than
        # restated. The digest list must exist in exactly one place: this
        # repository is public, so a second copy of it IS the disclosure it
        # guards against. What stays here is what only a test can do - enumerate
        # what the repository PUBLISHES, and refuse an exemption gone stale.
        #
        # The move is what makes the rules reachable from where the leak keeps
        # happening. A pull-request body is published to this same audience and
        # is not a tracked file, so this test could never have seen one; the
        # accepted-residual note above said as much before the module existed -
        # "what closes it is a private pre-publish scan, not a rule in this
        ...
        for name in listed.stdout.split("\0"):
            ...
            offenders.extend(
                scan(
                    name,
                    f"{name} (in the filename)",
                    observed=observed_hyphenated_term_counts,
                )
            )
            try:
            ...
            offenders.extend(scan(raw, name, observed=observed_hyphenated_term_counts))
        self.assertGreater(
        ...
        for _ in range(disclosure_scan.max_escape_normalization_passes):
            too_deeply_escaped = "%25{}".format(too_deeply_escaped[1:])
        normalized, exhausted, _ = disclosure_scan.decode_escaped_reference(
    ...
    def test_the_guide_sends_the_reader_to_the_reference_and_names_no_address(
        ...
        the heads measured then, `git merge-tree` auto-merged GUIDE.md and
        SKILL.md in both directions, and the only conflict was the generated
        behaviour lock - since retired, so today the same measurement would
        merge clean outright. So the hazard is not live today. That is the
        argument for a check rather than a narrative: a rebase retired it
    ...
    def test_the_retired_prompt_control_is_absent_from_the_tracked_tree(self) -> None:
        ...
        forbidden = (
            "_".join(("pre", "action", "reflect")),
            "-".join(("pre", "action")) + " reflect",
            "-".join(("pre", "action")) + " reflection",
        )
        ...
        def scan(files: dict[str, str]) -> list[str]:
            return sorted(
                ...
                if any(phrase in body.casefold() for phrase in forbidden)
            )

        self.assertEqual(scan(tracked_text_files()), [])
        self.assertEqual(
            scan({"planted.md": "Use " + forbidden[0] + " here."}),
            ["planted.md"],
            "the repository-wide absence guard no longer catches the retired control",
        )
    ...
    def test_sdk_template_defines_prompt_builder(self) -> None:
        ...
        # The helper supports the paid prompt controls, and each intervenes at
        # a different moment - `thinking_shape` during the answer and
        # `reflect` on the finished answer.
        # Two knobs that mean the
    ...
    def test_sdk_comparison_uses_twelve_rows_then_added_knobs_and_twelve_trials(
        ...
        # prompt styles x 2 thinking shapes, and 3 models x 3 binary knobs.
        count = namespace["configuration_count"]
        self.assertEqual(count(namespace["BASELINE_SPACE"]), 12)
        self.assertEqual(count(namespace["ENHANCED_SPACE"]), 24)
        self.assertEqual(len(namespace["BEHAVIOUR_KNOBS"]), 3)
        ...
        # Each supported prompt control intervenes at its own point and
        # produces a
        ...
        self.assertEqual(len(set(variants.values())), 3)
        ...
        self.assertEqual(count(reasoning_namespace["ENHANCED_SPACE"]), 24)
        self.assertEqual(
            reasoning_namespace["ENHANCED_SPACE"], namespace["ENHANCED_SPACE"]
        )

    def test_the_generated_space_is_not_mutated_between_paid_runs(self) -> None:
        """The first taste fixes its comparison before either paid phase.
        ...
        for phrase in (
            "all enhanced controls are already fixed",
            "fixes every generated value before either run",
            "all three controls and all their values are fixed before either run",
            "not an undisclosed edit to the approved comparison",
        ):
            with self.subTest(phrase=phrase):
                self.assertIn(phrase, f"{sdk} {safety}")

        for stale in (
            "baseline result chooses the enhanced space's values",
    ...
    def test_the_reduced_space_is_stated_exactly_and_framed_honestly(self) -> None:
        ...
        for phrase in (
            ...
            f"3 models × 3 binary behaviour knobs = {sizes['enhanced']} configurations",
        ):
            with self.subTest(phrase=phrase):
    ...
    # the enhanced space is written: 3 models x 3 binary knobs is 3 x 2**3.
    ...
    def _stated_factors(cls, phrase: str) -> list[int]:
        ...
        "3 binary knobs" is three knobs of two values each, not one knob of
        three, so it expands to `[2, 2, 2]`. The product is the same either
    ...
    def test_every_stated_space_arithmetic_multiplies_out_to_the_real_space(
        ...
        `sdk-execution.md` owns the two arithmetic statements. `run-safety.md`
        used to repeat them three times despite naming that owner; issue #215
        removes those restatements and keeps only conclusions plus a pointer.
        ...
        # Two owner statements, and the count is asserted because a regex that stops
        ...
        self.assertEqual(
            ...
            "another home for a decision `sdk-execution.md` owns; losing one "
        ...
        self.assertEqual(
            self._stated_factors("3 models x 3 binary knobs "), [3, 2, 2, 2]
        )
    ...
    def test_the_advisory_claim_matches_which_branches_actually_block(
        ...
        #201 keeps both readings and changes which states produce them. Three
        construction sites block - a document listing nothing, one listing
        settings while attesting none, and a read of the agent that established
        nothing it can vary - and one is advisory, for the state where neither a
        document nor a read reached the score at all.

        So the claim has to be scoped, and scoped by the RIGHT line: what
        separates the blocking branches from the advisory one is whether
        anything was looked at, not whether a document exists. A read that found
        nothing blocks, and it has no document.

        Still read off the module rather than pinned, and the direction is what
        the reading buys: a branch moving across that line fails here, because
        ...
        self.assertNotIn("`agent-no-varying-knobs` always blocks now", routing)
    ...
    def test_config_space_example_clears_the_cap_through_the_real_consumer(
        ...
        #
        # What earns the 100 is the shape rather than the taste: 24 distinct
        # configurations against a 12-trial budget, so the run compares twelve
        # of them, and 24 is twice the budget rather than twenty times it.
    ...
    def test_every_pair_of_configuration_counts_agrees_with_the_fence(self) -> None:
        ...
        updated the asserts and one bullet and left two sentences with the old
        pair - each correct-looking on its own, and neither visible in the diff
    ...
    def test_a_conditional_knob_loads_and_says_which_models_honour_it(self) -> None:
        ...
        self.assertEqual(
            ...
            # All three paid behaviour knobs act through the prompt, so a model
            # that takes no prompt shaping at all is asymmetric in all three.
            # It was two while the space also swept temperature.
            {"prompt_style", "thinking_shape", "reflect"},
...
class RowIdsHaveAFallbackWhenTheDatasetHasNoneTests(unittest.TestCase):
    ...
    # ledger total noticed, and every edit updates that as a matter of routine,
    # so a future change dropping the scope would go green. The behaviour
    # lock's digest used to notice too, and its retirement makes this pin MORE
    # necessary, not less: that was the other incidental signal, and it was
    # one relock away from silence anyway. The scheme was pinned and the
    # condition that makes it true was not.

    def test_the_line_scheme_is_confined_to_a_file_the_customer_wrote(
        self,
    ) -> None:
        """`line-<n>` is only an identifier where one row is one line."""
        self.assertIn(
            "use `line-<n>` only for a jsonl file the customer wrote",
            self._dataset(),
        )
    ...
    def test_a_converted_dataset_carries_an_id_stamped_at_conversion(
        ...
        """The id has to be computable, not merely required.

        Conversion is the only moment the customer's own row position is still
        known. Asking a later stage to recover it from the working copy names a
        source nothing preserved, which is an instruction that cannot be
        followed rather than a rule that is hard to follow.
        """
        dataset = self._dataset()
        self.assertIn(
            "each row gets its 1-based position in the file the customer holds, "
            "counting data rows only, as `row-<n>`",
            dataset,
        )
        self.assertIn("do not re-derive one from the working copy", dataset)

    def test_the_run_says_which_convention_the_ids_follow(self) -> None:
        """Two schemes counting different things need naming, or they collide."""
