...
| `traigent-runs/run-log.jsonl` (created during a run) | One line per distinct blocking failure or result-affecting warning, deduplicated, with when it first and last happened and whether it cleared. Written to explain a run that stopped, and never read back as input. It names the class of thing that failed, not the instance: no secret, no path, no id, and no text from your data. Ignored when the project uses Git, like the rest of `traigent-runs/` |
