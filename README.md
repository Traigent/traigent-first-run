...
and `NOTICE`, so its terms travel with the files that are copied into a project.
...
The guide and the Traigent SDK are separate works under separate terms. Apache-2.0 rights in this
repository grant no rights in the SDK; evaluate the SDK under the terms below.
