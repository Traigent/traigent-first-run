...
Readiness score (the card, the three pillars, bands, caps, blocked)
  ...
  evidence is missing rather than merely low. Three things hold them: too little
  of the score was measured at all, the evaluation method is the wrong kind of
  check for your agent's output, or nobody has read the expected answers the
  ...
  comparison rather than the dataset. None moves the
  number, because every measurement behind it stands; what is held is the
  verdict. None is a cap, so none stops the run, and the card names which
