...
Readiness score (the card, the three pillars, bands, caps, blocked)
  ...
  only a read of an input beside its answer can. That read is a sample of a few
  rows, so what it settles is that the answers held where they were looked at,
  never that they are all correct. Neither moves the
