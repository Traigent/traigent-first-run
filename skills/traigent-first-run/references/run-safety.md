...
| `agent_type` | `"general"`, `"rag"`, or `"code_gen"` | no | recorded on the score and passed through; absent or empty means `"general"`. It **changes no number**. It selected the high-impact catalog the removed `coverage` sub-score was graded against, and with that sub-score gone nothing in the scorer reads it - so an unrecognized non-empty string no longer leaves anything unmeasured, moves no score, and lowers no confidence. Declaring the wrong type costs nothing and is caught by nothing. A non-string is still refused |
