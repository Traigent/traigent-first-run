...
A project that arrives with real rows but fewer than 28 comparable ones gets that total offered to
it, and 28 is a ceiling on the offer rather than a target the run pursues by itself. Two states sit
outside it. A project that maintains its own held-out split keeps that split as it stands, per
