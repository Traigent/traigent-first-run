...
| Create or update a minimal `.env` | Proceed only after free checks, and only through `references/run-safety.md`'s ordered handoff, which selects the file. Preserve existing values and comments, append only its missing provider key (and, on `run-safety.md`'s copied-actor route A, the copied database file's absolute path under the name that route states), and set mode `0600` before opening as that handoff directs. Before writing, run that reference's git-tracked-file safety check and its ignore verification; it owns the exact commands and exit-code handling, and stop before secret entry if either check fails. Outside Git, do not create `.gitignore`. Never copy or request a duplicate key. Add or request the Traigent key only after the baseline checkpoint. |
...
If the scope gate refused an evaluator and a copy of it could be calibrated against a copied database file or a target the customer supplies: [`references/run-safety.md` § The copied-actor route](references/run-safety.md#the-copied-actor-route).
...
route applies the check is made on a copy against a safe target - a copy of their database file this run made, or one they supplied - and its result is
