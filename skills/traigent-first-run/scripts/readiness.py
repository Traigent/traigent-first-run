...
#
# It is worth saying what this deliberately does NOT fix. A project whose
# scorer executes candidate-generated SQL is held here by obeying the scope
# gate and scores far higher by running the calibration anyway, because the
# calibration sub-score and the probe spread are only earned by a probe that
# ran. That gap is an honest statement about evidence: the second run really
# did establish something the first did not. What is wrong is that this
# package offers no safe route to that evidence, so the only way to earn it is
...
def score_evaluation(facts: EvaluationFacts) -> tuple[Pillar, list[Cap]]:
    ...
    if (
        ...
        # The run said the scope gate refused this evaluator, so the sentence
        # below - "complete calibration" - is an instruction to do what
        # SKILL.md forbids. The condition after this one says the true thing
        # instead, at the same ceiling.
        and not facts.calibration_scope_refused
    ):
        caps.append(
            Cap(
                "evaluator-unvalidated",
    ...
    if (
        facts.method is not None
        and facts.calibration_scope_refused
        and not established
        and not disqualifying
        and facts.timed_out is not True
    ):
        caps.append(
            Cap(
                ...
                f"An evaluator method ({facts.method}) was declared, and this "
                "run did not calibrate it because running it here is outside "
                "the scope this guide permits. Nothing about the evaluator is "
...
def _settled_binding_region(
    ...
    """The subtree outside which a settled binding has no live read.

    Asked only of a binding `_settled_local_binding` has already settled, so
    there are two answers. A direct statement's region is the owner, and the
    question a caller asks with it is then trivially yes. A binding
    `_binding_block_cannot_continue` admits has the statement its raising block
    belongs to, because a read anywhere else finds the name unbound. Here
    rather than at the caller for the reason the rest of this home is: a reader
    that works the placement out for itself is one statement away from
    re-deriving the conditions around it.
    """
    parent = source.parents.get(id(node))
    return owner if parent is None else parent
...
def _settled_binding_is_read_after(
    ...
    a reader checks by looking; and it must sit inside
    `_settled_binding_region`, which for a binding in a raising block is that
    block's own statement. Without the second, a read written after the guard
    would be credited the value the guard bound - and that read can never
    execute with the name bound, so crediting it credits a setting no run of
    the agent ever varies.
    """
...
def _alias_reads_only_route_a_request(
    ...
    One placement answers yes without the enumeration being consulted, and it
    is not a shortcut - it is a STRONGER argument than the enumeration makes.
    When `_binding_block_cannot_continue` admits the binding, every read of the
    alias that can execute is inside a block that ends in a `raise`, so the
    call returns no request and no read of the alias can send, replace or
    mutate a payload. The enumeration below asks the weaker question of a read
    on a path that DOES reach the request, and asking it here would refuse
    `raise ValueError(f"missing {needed}")` - a read inside an error message,
    which is the reads residual this function records - for a value that
    provably never reaches one. That is the same argument
    ...
    Its bound is the bound that exemption already has: an expression evaluated
    while raising can still make a call of its own, and neither reader follows
    one into a request.
    """
    if (
        binding := _settled_local_assignment(alias, callable_node, source)
    ) is not None and _binding_block_cannot_continue(binding, callable_node, source):
        return True
    return all(
